...
void Space(void* p) {
    if(static_cast<CommonData*>(p)->camera.position.y <= 1.5) {
        auto camVel = static_cast<CommonData*>(p)->camera.vel;
        static_cast<CommonData*>(p)->camera.vel = camVel + v3<f32>{0,0.05,0};
    }
}
void Shift(void* p) {
...
void LoadSponza(RendererInterface* interface, Renderer* renderer , Scene& scene) {
    ...
    for(u32 i = 0; i < loader.LoadedMeshes.size() ; i++) {
        ...
        std::cout << loader.LoadedMeshes[i].MeshMaterial.map_Kd << std::endl;
...
void ExtractPlanesFromMat( Mat4<f32> mat, glm::vec4* left, glm::vec4* right, glm::vec4* top,glm::vec4* bottom,glm::vec4* near,glm::vec4* far) {
...
int main() {
    ...
    CommonData data;
    LoadRendererInterface(data.interface , "./renderer_software.so");
    ...
    Mat4<f32> proj = PerspectiveMatrix_(90.f, 16.f/9.f, 0.1f, 100.0f);
    ...
    while(data.open) {
        ...
        if( data.interface.ReadyToRender(renderer)) {
            ...
            v3<f32> pos;
            pos.x = data.camera.position.x;
            pos.y = data.camera.position.y;
            pos.z = data.camera.position.z;

            v3<f32> dir;
            dir.x = data.camera.direction.x;
            dir.y = data.camera.direction.y;
            dir.z = data.camera.direction.z;

            Mat4<f32> viewMat =  LookAt(pos, pos + dir);
            Mat4<f32> trasnformMat = proj * viewMat;
            
            data.interface.BeginRenderState(renderer , states[stateIndex]);
            auto& s = *data.interface.GetShaderParams(states[stateIndex]);

            memcpy(&s.projectionViewMatrix, &trasnformMat, sizeof(Mat4<f32>));
            data.interface.ReleaseShaderParams(states[stateIndex]);
            ExtractPlanesFromMat(trasnformMat , frustum,frustum+1,frustum+2,frustum+3,frustum+4,frustum+5);
        ...
        data.camera.vel = data.camera.vel + v3<f32>{0,-0.0001,0};
        data.camera.position = data.camera.position + data.camera.vel;
